...
dir_img = '../data/vangogh2photo/tra/'
dir_real = '../data/vangogh2photo/trb/'
dir_checkpoint = './checkpoints/'
path_checkpoint = './checkpoints/pretrained_epoch5.pth'
path_checkpointb = './checkpoints/checkpointDb_c2_epoch12.pth'
path_checkpointg = './checkpoints/checkpointDg_c2_epoch12.pth'
data_num = 0
...
def train(
    ...
    G.load_state_dict(torch.load(path_checkpoint))
    ...
    #D_blur.load_state_dict(torch.load(path_checkpointb))
    #D_gray.load_state_dict(torch.load(path_checkpointg))
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            if(num % 5 == 0):
                ...
                writer.add_scalar('train/all_loss', all_loss, it)
                writer.add_scalar('train/loss_sum', loss_sum, it)
                writer.add_scalar('train/loss_superpixel', loss_superpixel, it)
                writer.add_scalar('train/loss_content', loss_content, it)
                writer.add_scalar('train/loss_tv', loss_tv, it)

        print(epoch)
        torch.save(G.state_dict(), dir_checkpoint +f'checkpointD_or1_epoch{epoch+1}.pth')
        torch.save(D_blur.state_dict(), dir_checkpoint +f'checkpointDb_or1_epoch{epoch+1}.pth')
        torch.save(D_gray.state_dict(), dir_checkpoint +f'checkpointDg_or1_epoch{epoch+1}.pth')
